      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1.3fr", gap: "1.25rem" }}>
        ...
        <GlassCard style={{ padding: "1.25rem" }}>
          ...
          <div style={{ height: 320, borderRadius: "1rem", overflow: "hidden",
            ...
            <Suspense fallback={<div style={{ display: "flex", alignItems: "center", justifyContent: "center", height: "100%", color: "#22c55e", fontSize: "0.8rem" }}>Loading 3D...</div>}>
              <InteractiveBin level={selected.level / 100} />
            </Suspense>
          </div>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: "0.75rem", marginTop: "1rem" }}>
            {[
              { label: "Fill Level", value: `${selected.level}%`, color: statusConfig[selected.status]?.color },
              { label: "Waste Type", value: selected.type, color: typeColors[selected.type] },
              { label: "Last Update", value: selected.lastUpdated, color: "#06b6d4" },
            ].map(item => (
        ...
        <GlassCard style={{ padding: "1.25rem" }}>
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: "1rem" }}>
            ...
            <span style={{ fontSize: "0.7rem", color: "#22c55e", fontWeight: 600 }}>{bins.length} bins monitored</span>
          </div>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: "0.6rem", maxHeight: 520, overflowY: "auto" }}>
            {bins.map(bin => {
              ...
              const isSelected = bin.id === selected.id;
              return (
                <div
                  key={bin.id}
                ...
                >
                  <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: "0.5rem" }}>
                    <div>
                      ...
                      <div style={{ fontSize: "0.68rem", color: "#475569", marginTop: "2px" }}>{bin.id} · {bin.lastUpdated}</div>
                    </div>
                    <div style={{ display: "flex", flexDirection: "column", alignItems: "flex-end", gap: "4px" }}>
                      ...
                      <span style={{ fontSize: "0.68rem", color: typeColors[bin.type], fontWeight: 600 }}>{bin.type}</span>
                    </div>
                  </div>

...
export default function Dashboard() {
  ...
  const currentStats = {
    ...
    recycledToday: liveStats?.collected_kg ? `${liveStats.collected_kg} kg` : "0 kg",
    efficiencyScore: liveStats?.efficiency || 0
  };
    ...
    <PageLayout title="Dashboard">
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: "1rem", marginBottom: "1.5rem" }}>
        ...
        <GlassCard style={{ padding: "1.5rem", display: "flex", flexDirection: "column", justifyContent: "center" }}>
          ...
          <p style={{ fontSize: "2.2rem", fontWeight: 900, color: "#10B981", letterSpacing: "-0.02em" }}>{liveStats?.co2_saved_kg || 0} kg</p>
          <p style={{ fontSize: "0.75rem", color: "#64748B", marginTop: "0.3rem" }}>CO₂ equivalent via route opt.</p>
          <div style={{ marginTop: "0.75rem", height: 3, borderRadius: 9999, background: "linear-gradient(90deg, #10B98140, #10B981, #10B98140)", boxShadow: "0 0 8px #10B981" }} />
        </GlassCard>
      </div>
        ...
        <GlassCard>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: "0.75rem" }}>
            {currentTopBins.length > 0 ? currentTopBins.map(bin => {
              const color = bin.level >= 95 ? "#EF4444" : bin.level >= 80 ? "#F59E0B" : "#10B981";
              return (
                <div key={bin.id}>
                  <div style={{ display: "flex", justifyContent: "space-between", marginBottom: "0.25rem" }}>
                    <span style={{ fontSize: "0.78rem", color: "#475569", fontWeight: 700 }}>{bin.name.split(" – ")[0]}</span>

...
import { stats, alerts, wasteCategories, weeklyData, bins } from "../data/mockData";
...
export default function Dashboard() {
  const topBins = bins.filter(b => b.level >= 78).slice(0, 4);

  return (
    <PageLayout title="Dashboard">
      {/* Stat cards */}
      <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: "1rem", marginBottom: "1.5rem" }}>
        <StatCard title="Total Bins"    value={stats.totalBins}  color="#06b6d4" icon={Trash2}       subtitle="Active monitoring"    trend="up"   trendValue="+2 this week" />
        <StatCard title="Full Bins"     value={stats.fullBins}   color="#f97316" icon={AlertOctagon} subtitle="Needs collection"     trend="up"   trendValue="+3 today" />
        <StatCard title="Overflow Bins" value={stats.overflowBins} color="#ef4444" icon={Activity}  subtitle="Immediate action"     trend="down" trendValue="−1 from yesterday" />
        <StatCard title="Active Alerts" value={stats.activeAlerts} color="#22c55e" icon={Zap}        subtitle="Last 30 minutes"      trend="up"   trendValue="+2 new" />
      </div>

      {/* Row 2 */}
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: "1rem", marginBottom: "1.5rem" }}>
        <StatCard title="Recycled Today" value={stats.recycledToday} color="#a855f7" icon={Recycle} subtitle="Tonnes diverted" trend="up" trendValue="+0.3T vs yesterday" />
        <StatCard title="Efficiency Score" value={`${stats.efficiencyScore}%`} color="#22c55e" icon={TrendingDown} subtitle="Collection efficiency" trend="up" trendValue="+4% this week" />
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1.4fr 1fr 1fr", gap: "1rem", marginBottom: "1.5rem" }}>
        ...
        <GlassCard>
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: "1rem" }}>
            ...
            <span style={{ fontSize: "0.7rem", color: "#ef4444", fontWeight: 600,
              ...
              borderRadius: 9999, padding: "0.2rem 0.6rem" }}>{alerts.length} active</span>
          </div>
        ...
        {/* Quick bin overview */}
        <GlassCard>
          <h2 style={{ fontSize: "1rem", fontWeight: 700, marginBottom: "1rem" }}>Priority Bins</h2>
